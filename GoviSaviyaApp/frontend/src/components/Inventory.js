...
function Inventory(){
    ...
    const[searchTitle,setSearchTitle]=useState("");
      ...
      useEffect(()=>{
        const loadPosts =async () =>{
            setLoading(true);
            const response = await axios.get("http://localhost:8000/seedroute/");
    ...
    console.warn(seed)
    function getSeed() {
        fetch("http://localhost:8000/seedroute/").then((result) => {
     ...
     function deleteSeed(id) {
         if (window.confirm('Are You Sure?')) {
            ...
            fetch('http://localhost:8000/seedroute/delete/' + id, {
                method: 'DELETE',
            }).then((result) => {
                result.json().then((resp) => {
                    console.warn(resp)
    ...
    return(
        ...
        <button className="addseed"><NavLink className="add-nav" activeClassName="active" to="/addseed">Add New Seed</NavLink></button>
