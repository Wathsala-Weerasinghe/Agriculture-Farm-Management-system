...
useEffect(()=>{
  const loadPosts = async()=>{
    setloading(true);
    const response =await axios.get("http://localhost:8000/confirm/c/");
  ...
  loadPosts();
    ...
    useEffect(()=>{
        function getconfs(){
            axios.get("http://localhost:8000/confirm/c/").then((res)=>{
                setConfs(res.data);
            }).catch((err)=>{
                alert(err.message);
            })
    ...
    function deleteConf(_id){
        axios.delete("http://localhost:8000/confirm/delete/"+_id).then((res)=>{
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <Link to={"/addconfirm"} className="btn btn-success btn-sm">Add New Order</Link>
